...
function populateJobs(params) {
    ...
    if (dataToBeDisplayed && dataToBeDisplayed.length) {
        ...
        pageDescriptionSpan.innerText = `${recordStartIndex + 1} - ${recordStartIndex + dataToBeDisplayed.length
...
function toAbbreviation(stateName) {
    ...
    try {
        if (fetchJobsFromAPI) {
            ...
            if (!allJobs || (allJobs && allJobs.length == 0))
                ...
                allJobs.forEach(job => {
                    job.state = toAbbreviation(job.state);
                });
